...
export function LoadedVehiclesPage() {
  ...
  return <div><PageHeader title="Loaded Vehicles" description="Vehicles are physically loaded, then marked READY." />
    ...
    {loading?<LoadingState message="Loading existing optimized vehicles..."/>:!plan?<EmptyState icon={PackageOpen} title="No load plan selected" description="Complete Parcel Consolidation and Load Optimization first." action={<PrimaryButton onClick={()=>navigate('/parcel-consolidation')}>Go to Parcel Consolidation</PrimaryButton>}/>:<><div className="mb-5 grid gap-3 sm:grid-cols-5"><Summary label="Vehicles shown" value={`${vehicles.length} / ${plan.n_vehicles}`}/><Summary label="Ready" value={`${ready} / ${plan.n_vehicles}`}/><Summary label="Mean utilization" value={formatPercent(plan.mean_utilization)}/><Summary label="Cost / parcel" value={`LKR ${formatNumber(plan.cost_per_parcel)}`}/><Summary label="Parcels" value={formatNumber(plan.n_parcels)}/></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{vehicles.map((v)=><VehicleCard key={v.virtual_vehicle_id} vehicle={v} pending={pending.has(v.virtual_vehicle_id)} onReady={()=>markReady(v)} onView={()=>{setShowIds(false);setViewing(v)}}/>)}</div></>}
