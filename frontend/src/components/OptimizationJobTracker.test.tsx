...
vi.mock('../services/optimizationService', () => ({ optimizationService: { listJobs: vi.fn(), cancelJob: vi.fn(), deleteJob: vi.fn() } }))
...
const running = { job_id:'JOB-1', status:'RUNNING', job_type:'SINGLE_CLUSTER', batch_id:null, cluster_id:4,
  ...
  result_summary:null, error_code:null, error_message:null, cancel_requested:false, created_at:'2026-01-05T00:00:00',
...
describe('OptimizationJobTracker', () => {
  ...
  it('cancels a running job and hides the delete button while it is active', async () => {
    vi.mocked(optimizationService.listJobs).mockResolvedValue([running])
    vi.mocked(optimizationService.cancelJob).mockResolvedValue({...running, cancel_requested:true})
    render(<MemoryRouter><OptimizationJobTracker /></MemoryRouter>)
    fireEvent.click(await screen.findByRole('button',{name:'Optimization jobs'}))
    expect(screen.queryByRole('button',{name:'Delete'})).not.toBeInTheDocument()
    fireEvent.click(screen.getByRole('button',{name:'Cancel'}))
    await waitFor(()=>expect(optimizationService.cancelJob).toHaveBeenCalledWith('JOB-1'))
  })

  it('deletes a finished job from the list', async () => {
    const completed = {...running, status:'COMPLETED' as const, plan_id:'PLAN-1'}
    vi.mocked(optimizationService.listJobs).mockResolvedValue([completed])
    vi.mocked(optimizationService.deleteJob).mockResolvedValue(undefined)
    render(<MemoryRouter><OptimizationJobTracker /></MemoryRouter>)
    fireEvent.click(await screen.findByRole('button',{name:'Optimization jobs'}))
    expect(screen.queryByRole('button',{name:'Cancel'})).not.toBeInTheDocument()
    fireEvent.click(screen.getByRole('button',{name:'Delete'}))
    await waitFor(()=>expect(optimizationService.deleteJob).toHaveBeenCalledWith('JOB-1'))
    await waitFor(()=>expect(screen.queryByText(/Cluster 4/)).not.toBeInTheDocument())
  })
})
