...
const ACTIVE_STATUSES: OptimizationJob['status'][] = ['QUEUED', 'RUNNING']
...
export function OptimizationJobTracker() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        setJobs(rows)
        const hasActive = rows.some((job) => ACTIVE_STATUSES.includes(job.status))
  ...
  const cancel = async (jobId: string) => {
    setBusyJobId(jobId)
    try {
      await optimizationService.cancelJob(jobId)
  ...
  const remove = async (jobId: string) => {
    setBusyJobId(jobId)
    try {
      await optimizationService.deleteJob(jobId)
  ...
  return <div className="relative">
    ...
    {open&&<div className="absolute right-0 top-12 z-50 w-96 max-w-[85vw] rounded-2xl border bg-white p-4 shadow-2xl"><h3 className="mb-3 font-black">Recent optimization jobs</h3><div className="max-h-80 space-y-3 overflow-auto">{jobs.slice(0,10).map((job)=>{
      const isActive = ACTIVE_STATUSES.includes(job.status)
      const isBusy = busyJobId === job.job_id
      return <div className="rounded-xl border p-3 text-xs" key={job.job_id}>
        <div className="flex justify-between"><b>{job.cluster_id===null?'Parcel set':`Cluster ${job.cluster_id}`}</b><b>{job.status}</b></div>
        <div className="mt-1 text-fleet-muted">{job.stage} · {job.progress_percent}% — {job.cancel_requested&&job.status==='RUNNING'?'Cancellation requested; stopping soon':job.message}</div>
        <div className="mt-2 h-1.5 overflow-hidden rounded bg-slate-100"><div className="h-full bg-fleet-blue" style={{width:`${job.progress_percent}%`}} /></div>
        {job.plan_id&&<Link className="mt-2 inline-block font-black text-fleet-blue" to={`/load-plans/${job.plan_id}`}>View Plan</Link>}
        {job.status==='FAILED'&&<p className="mt-2 text-red-700">{job.error_message}</p>}
        <div className="mt-2 flex justify-end gap-2">
          {isActive&&<button className="rounded-lg border border-red-200 px-2 py-1 font-bold text-red-700 disabled:opacity-50" disabled={isBusy||job.cancel_requested} onClick={()=>cancel(job.job_id)} type="button">
            {job.cancel_requested?'Cancelling…':isBusy?'Cancelling…':'Cancel'}
          </button>}
          {!isActive&&<button className="rounded-lg border border-fleet-line px-2 py-1 font-bold text-fleet-muted disabled:opacity-50" disabled={isBusy} onClick={()=>remove(job.job_id)} type="button">
