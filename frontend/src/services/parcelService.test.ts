...
describe('parcel planning-instance requests', () => {
  ...
  it('clusters by depot/date without dataset_id', async () => {
    const fetchMock = vi.fn().mockResolvedValue(new Response(JSON.stringify({ status: 'trained', parcel_count: 400, n_clusters_pre_repair: 4, n_clusters_post_repair: 3, noise_count: 2, unassigned_count: 0, runtime_seconds: 1, repair: { applied: true, n_split: 0, n_merged: 1, excluded_infeasible_count: 0 }, clusters: {} }), { status: 200, headers: { 'Content-Type': 'application/json' } }))
    vi.stubGlobal('fetch', fetchMock)
