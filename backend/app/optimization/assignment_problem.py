...
class AssignmentConfig:
    ...
    min_parcels_per_vehicle: int = 5
    max_vehicle_slots: int = 40
    ...
    # (midnight) would make every daytime window unreachable.
...
def slot_budget(n_parcels: int, config: AssignmentConfig, n_warm_clusters: int = 0) -> int:
    """K, the number of vehicle slots offered to the GA. Large enough to
    hold a capacity-derived estimate of how many vehicles are needed, and
    at least as many as any warm-start cluster set so every repaired
    cluster can seed its own slot; bounded so the search space stays
    tractable."""
    estimate = math.ceil(n_parcels / config.min_parcels_per_vehicle)
    return max(1, min(config.max_vehicle_slots, max(estimate, n_warm_clusters)))
...
def schedule_time_window_compliance(
    ...
    for parcel in parcels_in_delivery_order:
        ...
        compliant.append(start <= clock_minutes <= end)
    ...
    return sum(compliant) / len(compliant), compliant
...
def vehicle_metrics(parcel_objs: list, vehicle: VehicleTypeSpec, config: AssignmentConfig) -> dict:
    ...
    compliance, _flags = schedule_time_window_compliance(ordered_parcels, vehicle, config)
...
class AssignmentProblem(Problem):
    def __init__(self, parcels: list, catalog: tuple[VehicleTypeSpec, ...], config: AssignmentConfig, n_slots: int | None = None):
        ...
        self.config = config
        self.n = len(parcels)
        self.K = n_slots if n_slots is not None else slot_budget(self.n, config)
        self.T = len(catalog)
    ...
    def evaluate_individual(self, row) -> tuple[np.ndarray, np.ndarray]:
        ...
        for slot_idx, parcel_indices in slots.items():
            ...
            m = vehicle_metrics(parcel_objs, vehicle, self.config)
            ...
            if attempt_placement(m["ordered_parcels"], vehicle) is None:
        ...
        mean_compliance = float(np.mean([m["compliance"] for m in used_metrics]))
...
class OverflowRepair(Repair):
    ...
    def _repair_row(self, problem: AssignmentProblem, row):
        ...
        for slot_idx, parcel_indices in list(slots.items()):
            type_idx = int(np.clip(type_of_slot[slot_idx], 0, problem.T - 1))
            vehicle = problem.catalog[type_idx]
            members = [(i, problem.parcels[i]) for i in parcel_indices]
            weight = sum(p.weight_kg for _, p in members)
            volume = sum(p.volume_m3 for _, p in members)
            if weight <= vehicle.capacity_kg and volume <= vehicle.capacity_m3:
                continue

            members.sort(key=lambda ip: ip[1].weight_kg)
            for idx, parcel in members:
                if weight <= vehicle.capacity_kg and volume <= vehicle.capacity_m3:
                    break
                target = self._least_loaded_compatible_slot(problem, row, slot_idx, parcel)
                if target is None:
                    continue
                row[idx] = target
                weight -= parcel.weight_kg
                volume -= parcel.volume_m3

    def _least_loaded_compatible_slot(self, problem: AssignmentProblem, row, exclude_slot: int, parcel):
        slots, type_of_slot = decode(row, problem.n, problem.K)
        ...
        for slot_idx in range(problem.K):
            ...
            type_idx = int(np.clip(type_of_slot[slot_idx], 0, problem.T - 1))
            vehicle = problem.catalog[type_idx]
            members = [problem.parcels[i] for i in slots.get(slot_idx, [])]
            weight = sum(p.weight_kg for p in members)
            volume = sum(p.volume_m3 for p in members)
            if weight + parcel.weight_kg > vehicle.capacity_kg or volume + parcel.volume_m3 > vehicle.capacity_m3:
                continue
            load_fraction = max(
...
def run_nsga2(
    ...
    *,
    ...
    K = slot_budget(len(parcels), config, n_warm_clusters)
